...
function App() {
  ...
  function getData() {
    ...
    fetch(`http://localhost:5000/todos/alltodos`, { method: "GET" }) // mandando uma requisição GET para a api
      .then((response) => response.json()) // api responde e o o primeiro then pega a data que veio da resposta da api e transforma em json
  ...
  function insertData(todotitle, todocategory) {
    ...
    fetch(`http://localhost:5000/todos/add`, {
      ...
      }), // transformado os dados do body em json
    })
      .then((response) => response.json()) // pegando a resposta do servidor em JSON
      .then((data) => getData()); // chamando a funçao getData para atualizar o estado de acordo com o post feito
  }
  ...
  function updateData(todo) {
    ...
    newTodos.map((item) =>
      item._id === todo._id ? (item.active = !item.active) : null
    );
    setTodos(newTodos);
    ...
    fetch(`http://localhost:5000/todos/${todo._id}`, {
      ...
      }, // declarando que o request enviado para o servidor será um JSON
      body: JSON.stringify({
        // transformado os dados do body em json
        active: `${todo.active}`,
      }),
      ...
      .then((response) => response.json()) // pegando a resposta do servidor em JSON
      .then((data) => getData())
      .catch((error) => console.error(error)); // chamando a funçao getData para atualizar o estado de acordo com o post feito
  }

  function deleteData(todo) {
    const newTodos = [...todos];
    const filteredTodos = newTodos.filter((item) =>
      item._id === todo._id ? null : todo
    );
